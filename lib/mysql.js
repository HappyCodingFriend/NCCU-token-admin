...
function addPoint(address, name, owner, deadline) {
    console.log(new Date(deadline).toISOString().slice(0, 19).replace('T', ' '))
    let cmd = "INSERT INTO point (address, name, owner, deadline) VALUES ?";
    connection.query(cmd, [[[address, name, owner, deadline]]], (err, result) => {
        if (err) {
            console.error(err)
        }
    })
