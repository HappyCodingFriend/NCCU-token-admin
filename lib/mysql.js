...
function addTransaction(txhash, from, to) {
    let cmd = "INSERT INTO transaction (txHash, ID, point) VALUES ?";
...
function addOrder(address, owner) {
    let cmd = "INSERT INTO  (address, owner) VALUES ?";
...
function getOrderIsNotFinish(cb) {
    let cmd = "SELECT * FROM order WHERE buyer = ''";
    connection.query(cmd, (err, result) => {
        if (err)
            console.error(err)
        else {
...
function finishOrder(address, cb) {
    let cmd = "UPDATE order SET buyer ='1' WHERE address = ?";
